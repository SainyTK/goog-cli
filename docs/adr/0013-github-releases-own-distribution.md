...
Every `goog` invocation checks the canonical latest stable GitHub Release in parallel with command execution.
Successful results are cached locally for 24 hours, and failed checks are retried after one hour.
When a newer semantic version is known, the CLI prints its version and the public installer command to stderr after all command output, including help, version, and error paths.
