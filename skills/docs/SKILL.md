...
- `goog docs compare` for source-to-target inventory, visual-system, formatting, and mapped-content acceptance checks.
...
- Retain the final JSON acceptance record because it identifies both document IDs, titles, and edit URLs, the resolved account, running executable SHA-256, when acceptance occurred, the exact source and copied revisions, the revision-pinned replay command, and the four matched scope fingerprints that passed fidelity verification.
- The generated executable-hash-guarded and revision-guarded replay command has been retained with fidelity evidence so a rebuilt binary or later document edit cannot be mistaken for the accepted state.
- The comparison evidence records its stable report type and schema version so automation can identify the evidence before rejecting or migrating report shapes it does not understand.
- The comparison evidence records the account that accessed each document, and its replay command pins either their shared account or each account independently.
