...
JSON reports expose the emitted and limit-truncated path counts through `displayedDifferenceCount` and `differenceCountHiddenByLimit`.
Filtered JSON reports also expose the matching and out-of-filter split through `previewDifferenceCount` and `differenceCountOutsidePreview` for every selected scope.
