...
Each report records a revision-guarded replay command, the UTC comparison time, the goog CLI version and executable SHA-256, the selected scope, acceptance-gate setting, preview limit, summary mode, an explicit account override when supplied, and identifies the source and target by title, document ID, edit URL, revision ID, and the account that accessed it.
...
Keep the replay command, timestamp, CLI version, executable SHA-256, and revision IDs with acceptance evidence so a later document edit, tool change, or informational comparison cannot be mistaken for the tested acceptance gate.
