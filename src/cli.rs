...
Notes:
  ...
  Verified acceptance records identify both document IDs, Google-confirmed titles, and edit URLs, the resolved account, goog CLI version, execution OS and architecture, running executable path and SHA-256, acceptance time, the exact source and copied revisions, and the four matched scope fingerprints.
