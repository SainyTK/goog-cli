...
pub(super) async fn run_copy_to<S: AccountStore>(
    ...
    validate_executable_sha256(command.required_executable_sha256.as_deref())?;
    let goog_cli_executable_path = command
        .json
        .then(goog_cli_executable_path)
        .transpose()?
        .map(str::to_owned);
    ...
    if command.json {
        write_json_line(
            ...
            &CopyDocumentAcceptance {
                ...
                comparison_report_type: command
                    .verify_fidelity
                    .then_some(DOCUMENT_COMPARISON_REPORT_TYPE),
...
struct CopyDocumentAcceptance<'a> {
    ...
    comparison_report_type: Option<&'static str>,
...
const DOCUMENT_COMPARISON_REPORT_TYPE: &str = "goog.docs.compare";
...
pub(super) fn write_document_comparison_with_settings(
    ...
    if json {
        ...
        writeln!(out, "Report type: {}", report.report_type)
            .context("failed to write Docs comparison report type")?;
        writeln!(
            out,
