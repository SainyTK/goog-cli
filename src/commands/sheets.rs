...
pub(super) async fn run_sheet_to<S: AccountStore>(
    ...
    match cmd {
        ...
        SheetsSheetCommand::DeleteDuplicates {
            ...
            let request_body = delete_duplicates_sheet_request_body(
                sheet_id,
                start_row,
                end_row,
                start_column,
            ...
            let response = SheetsOperation::BatchUpdateSpreadsheet(&options)
                .execute(client)
                .await
                .context("failed to delete Google Sheets duplicate rows")?;
            write_json_line(
                out,
                &response,
        ...
        SheetsSheetCommand::RandomizeRange {
            ...
            let request_body = randomize_range_sheet_request_body(
                sheet_id,
                start_row,
                end_row,
                start_column,
            ...
            let response = SheetsOperation::BatchUpdateSpreadsheet(&options)
                .execute(client)
                .await
                .context("failed to randomize Google Sheets range")?;
            write_json_line(
                out,
                &response,
                "failed to serialize Sheets Randomize range response",
            )
        }
    ...
    match cmd {
        ...
        SheetsValuesCommand::AppendColumn {
            ...
            let response = SheetsOperation::AppendValues(&options)
                .execute(client)
                .await
                .context("failed to append Google Sheets column")?;
            write_json_line(
                out,
                &response,
                "failed to serialize Sheets Append column response",
            )
        }
...
fn delete_duplicates_sheet_request_body(
    sheet_id: i64,
    start_row: i64,
    end_row: i64,
    start_column: i64,
    end_column: i64,
    comparison_columns: &[i64],
) -> Result<serde_json::Value> {
    validate_grid_range(start_row, end_row, start_column, end_column)?;
    for comparison_column in comparison_columns {
        if *comparison_column < start_column || *comparison_column >= end_column {
            bail!("--comparison-column must be inside the selected column range");
        }
    }

    let mut delete_duplicates = serde_json::json!({
        "range": grid_range(sheet_id, start_row, end_row, start_column, end_column)
    });
    if !comparison_columns.is_empty() {
        delete_duplicates["comparisonColumns"] = serde_json::Value::Array(
            comparison_columns
                .iter()
                .map(|column| {
                    serde_json::json!({
...
fn randomize_range_sheet_request_body(
    sheet_id: i64,
    start_row: i64,
    end_row: i64,
    start_column: i64,
...
fn write_first_row_line(out: &mut impl Write, value_range: &serde_json::Value) -> Result<()> {
    let Some(row) = value_range
        .get("values")
        .and_then(|values| values.as_array())
        .and_then(|rows| rows.first())
        .and_then(|row| row.as_array())
