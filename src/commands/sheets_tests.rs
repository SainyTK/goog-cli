...
#[tokio::test]
async fn run_values_get_cell_prints_first_cell_as_scalar_line() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(header("authorization", "Bearer sheets-access"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
    ...
    assert_eq!(String::from_utf8(out).unwrap(), "=SUM(B2:B4)\n");

    let url = received_url(&server).await;
    assert!(url
        .path()
        .ends_with("/spreadsheets/spreadsheet-123/values/Sheet1!D2"));
    assert_eq!(
...
#[tokio::test]
async fn run_values_get_cell_prints_blank_line_for_empty_cell() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(header("authorization", "Bearer sheets-access"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
...
#[tokio::test]
async fn run_values_get_row_prints_first_row_as_tab_separated_line() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(header("authorization", "Bearer sheets-access"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "range": "Sheet1!A2:C2",
            "values": [["Ada", 42, "=SUM(B2:B4)"]]
        })))
        .expect(1)
        .mount(&server)
        .await;
    ...
    assert_eq!(String::from_utf8(out).unwrap(), "Ada\t42\t=SUM(B2:B4)\n");

    let url = received_url(&server).await;
    assert!(url
        .path()
        .ends_with("/spreadsheets/spreadsheet-123/values/Sheet1!A2:C2"));
    assert_eq!(
        query_value(&url, "valueRenderOption").as_deref(),
        Some("FORMULA")
    );
...
#[tokio::test]
async fn run_values_get_row_prints_blank_line_for_empty_row() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(header("authorization", "Bearer sheets-access"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
...
#[tokio::test]
async fn run_values_get_table_prints_tab_separated_rows() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(header("authorization", "Bearer sheets-access"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
    ...
    assert!(url
        .path()
        .ends_with("/spreadsheets/spreadsheet-123/values/Sheet1!A1:C3"));
    assert_eq!(
        query_value(&url, "valueRenderOption").as_deref(),
        Some("FORMULA")
    );
...
#[tokio::test]
async fn run_values_get_table_prints_blank_line_for_empty_table() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(header("authorization", "Bearer sheets-access"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
...
#[tokio::test]
async fn run_sheet_randomize_range_builds_randomize_range_batch_update() {
    let server = MockServer::start().await;
    let request_body = serde_json::json!({
        "requests": [
            {
...
#[tokio::test]
async fn run_sheet_randomize_range_rejects_empty_range() {
    let store = MemoryStore::default();
    let client = write_test_client(&store);
    let mut out = Vec::new();
