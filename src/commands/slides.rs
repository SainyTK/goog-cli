...
pub fn run<S: AccountStore>(
    ...
    match cmd {
        ...
        SlidesCommand::TableInsertColumns {
            ...
            number,
            right,
        } => run_with_runtime(run_table_insert_columns_unified_to(
            config,
            store,
            account_override,
            TableInsertColumnsRequest {
                presentation_id,
                table_id,
                reference_row,
                reference_column,
        ...
        SlidesCommand::TableDeleteColumn {
            ...
            reference_row,
            reference_column,
        } => run_with_runtime(run_table_delete_column_unified_to(
            config,
            store,
            account_override,
        ...
        SlidesCommand::TableUnmergeCells {
            ...
            row_span,
            column_span,
        } => run_with_runtime(run_table_unmerge_cells_unified_to(
            config,
            store,
            account_override,
            TableUnmergeCellsRequest {
                presentation_id,
                table_id,
                start_row,
                start_column,
...
#[derive(Debug, Clone)]
pub(super) struct TableInsertColumnsRequest {
    pub presentation_id: String,
    pub table_id: String,
    pub reference_row: u32,
    pub reference_column: u32,
...
pub(super) async fn run_table_insert_columns_unified_to<S: AccountStore>(
    config: &Config,
    store: &S,
    account_override: Option<&str>,
    request: TableInsertColumnsRequest,
    out: &mut impl Write,
    presentations_url: Option<&str>,
    state_path: Option<&Path>,
) -> Result<()> {
    let presentation_id = request.presentation_id.clone();
    let request_body = build_table_insert_columns_batch_update(request)?;
    let mut options = BatchUpdatePresentationOptions::new(presentation_id.clone(), request_body);
    if let Some(presentations_url) = presentations_url {
        options = options.with_presentations_url(presentations_url);
    }
    ...
    .await
    .context("failed to insert Google Slides table columns")?;

    write_json_line(
        out,
        &response,
        "failed to serialize Slides table column insertion response",
    )
}

pub(super) fn build_table_insert_columns_batch_update(
    request: TableInsertColumnsRequest,
) -> Result<serde_json::Value> {
    if request.number == 0 || request.number > 20 {
        bail!("slides table-insert-columns --number must be between 1 and 20");
    }
...
#[derive(Debug, Clone)]
pub(super) struct TableDeleteColumnRequest {
    pub presentation_id: String,
    pub table_id: String,
    pub reference_row: u32,
    pub reference_column: u32,
...
pub(super) async fn run_table_delete_column_unified_to<S: AccountStore>(
    config: &Config,
    store: &S,
    account_override: Option<&str>,
    request: TableDeleteColumnRequest,
    out: &mut impl Write,
    presentations_url: Option<&str>,
    state_path: Option<&Path>,
) -> Result<()> {
    let presentation_id = request.presentation_id.clone();
    let request_body = build_table_delete_column_batch_update(request);
    let mut options = BatchUpdatePresentationOptions::new(presentation_id.clone(), request_body);
    if let Some(presentations_url) = presentations_url {
        options = options.with_presentations_url(presentations_url);
    }
    ...
    .await
    .context("failed to delete Google Slides table column")?;

    write_json_line(
        out,
        &response,
        "failed to serialize Slides table column deletion response",
    )
}

pub(super) fn build_table_delete_column_batch_update(
    request: TableDeleteColumnRequest,
) -> serde_json::Value {
    serde_json::json!({
        "requests": [
...
#[derive(Debug, Clone)]
pub(super) struct TableUnmergeCellsRequest {
    pub presentation_id: String,
    pub table_id: String,
    pub start_row: u32,
    pub start_column: u32,
...
pub(super) async fn run_table_unmerge_cells_unified_to<S: AccountStore>(
    config: &Config,
    store: &S,
    account_override: Option<&str>,
    request: TableUnmergeCellsRequest,
    out: &mut impl Write,
    presentations_url: Option<&str>,
    state_path: Option<&Path>,
) -> Result<()> {
    let presentation_id = request.presentation_id.clone();
    let request_body = build_table_unmerge_cells_batch_update(request)?;
    let mut options = BatchUpdatePresentationOptions::new(presentation_id.clone(), request_body);
    if let Some(presentations_url) = presentations_url {
        options = options.with_presentations_url(presentations_url);
    }
    ...
    .await
    .context("failed to unmerge Google Slides table cells")?;

    write_json_line(
        out,
        &response,
        "failed to serialize Slides table cell unmerge response",
    )
}

pub(super) fn build_table_unmerge_cells_batch_update(
    request: TableUnmergeCellsRequest,
) -> Result<serde_json::Value> {
    if request.row_span == 0 || request.column_span == 0 {
        bail!("slides table-unmerge-cells spans must be greater than zero");
    }
