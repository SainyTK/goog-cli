...
const LATEST_RELEASE_URL: &str = "https://api.github.com/repos/SainyTK/goog-cli/releases/latest";
const INSTALL_COMMAND: &str =
    "curl -fsSL https://raw.githubusercontent.com/SainyTK/goog-cli/main/install.sh | sh";
const SUCCESS_CACHE_TTL: Duration = Duration::from_secs(24 * 60 * 60);
...
fn check() -> Option<UpdateNotice> {
    ...
    let latest = if should_use_cache(&cache, now) {
...
fn fetch_latest_release() -> Option<Version> {
    let release_url =
        std::env::var("GOOG_UPDATE_CHECK_URL").unwrap_or_else(|_| LATEST_RELEASE_URL.into());
...
fn should_use_cache(cache: &UpdateCache, now: u64) -> bool {
    cache
        .last_checked_at
        .is_some_and(|checked| now.saturating_sub(checked) < SUCCESS_CACHE_TTL.as_secs())
        || cache
            .last_attempt_at
